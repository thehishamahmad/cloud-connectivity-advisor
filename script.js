...
function determineRecommendation(fit, raw) {
  ...
  if (winner === "iap") {
    return {
      ...
      badgeColor: "#10b981",
      summary: "This environment does not appear to require continuous network-level hybrid connectivity. A simpler secure access model is likely sufficient.",
      why: "IAP-based access is recommended because the environment appears to have limited hybrid traffic dependency, low download or upload requirements, and a stronger need for simplicity and cost control. This approach avoids unnecessary VPN tunnels, routing complexity, and additional network operations overhead.",
      observations: buildObservations("iap", fit, raw),
      architecture: "Use Identity-Aware Proxy with IAM-based access control for SSH, RDP, or internal administrative web access. Keep workloads private where possible and avoid exposing management interfaces publicly.",
      nextStep: "Validate the access use case, define IAM roles, confirm administrator groups, and test secure access through IAP before considering VPN connectivity."
    };
  }
...
function buildObservations(strategy, fit) {
  ...
  if (strategy === "iap") {
    ...
    observations.push("Best suited for secure administrative access rather than application-to-application hybrid traffic.");
  }
